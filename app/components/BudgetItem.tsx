...
const BudgetItem: React.FC<BudgetItemProps> = ({ budget, enableHover }) => {
  ...
  return (
    <li
      key={budget.id}
      className={`p-4 rounded-xl border-2 border-base-300 list-none transition duration-300 ${hoverClasse} `}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className="bg-accent/20 border border-accent text-xl h-10 w-10 rounded-full flex justify-center items-center">
            {budget.emoji || "💰"}
          </div>
        ...
        <div
          ...
          className="inline-flex items-center rounded-full border bg-accent px-2 py-1 text-xl font-medium inset-ring inset-ring-accent/20"
        >
          {budget.amount} €
        </div>
      </div>
      <div className="flex justify-between items-center mt-4 text-gray-500 text-sm">
        <span>
          {totalTransactionAmount} € dépensé{totalTransactionAmount > 0 && "s"}
        </span>
        <span>
          {remainingAmount} € restant{remainingAmount > 0 && "s"}
        </span>
      </div>
